...
namespace Functions.Infrastructure
{
    ...
    internal static class ServiceProviderConfiguration
    {
        ...
        {
            string dir = Directory.GetParent(Directory.GetParent(new Uri(Assembly.GetExecutingAssembly().CodeBase).LocalPath).FullName).FullName;
            var config = new ConfigurationBuilder()

                .SetBasePath(dir)
                .AddJsonFile("ConfigFiles/settings.json", true, true)
                //#if DEBUG
